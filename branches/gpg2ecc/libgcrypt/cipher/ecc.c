...
/*
 * First obtain the setup.  Over the finite field randomize a scalar
              ...
              gcry_mpi_t q_x, gcry_mpi_t q_y)
{
  ...
  d = gen_k (E.n, is_long_term ? GCRY_VERY_STRONG_RANDOM : GCRY_STRONG_RANDOM); 
                  ...
                  gcry_sexp_t *r_extrainfo)
{
  ...
  if (genparms)
    {
      ...
      l1 = gcry_sexp_find_token (genparms, "ephemeral", 0);
      if( l1 )  {
         const char *s;
        s = _gcry_sexp_nth_string (l1, 1);
         ...
         if (DBG_CIPHER)
           log_debug( "ecgen 'ephemeral' parameter supplied, value=%d\n", is_long_term==0 );
       }
  ...
  q_x = mpi_new (0);
  q_y = mpi_new (0);
  ec = generate_key (&sk, nbits, curve_name, is_long_term, g_x, g_y, q_x, q_y);
