...
pk_ecdh_encrypt (gcry_mpi_t * resarr, const byte pk_fp[MAX_FINGERPRINT_LEN], gcry_mpi_t data, gcry_mpi_t * pkey)
{
  ...
  rc = pk_ecc_keypair_gen( &pk_eph, &sk_eph, PUBKEY_ALGO_ECDH, 0/*this is ephemeral*/, nbits );
  if( rc )
    return rc;
  if( DBG_CIPHER )  {
	unsigned char *buffer;
	if (gcry_mpi_aprint (GCRYMPI_FMT_HEX, &buffer, NULL, sk_eph->skey[1]))
          BUG ();
  ...
  if (gcry_sexp_build (&s_data, NULL, "%m", sk_eph->skey[3]))	/* ephemeral scalar goes as data */
    BUG ();
...

  {
    ...
    resarr[0] = sk_eph->skey[1];	/* ephemeral public key */
